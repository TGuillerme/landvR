...
An empirical example of this work is currently in review but can be found through [Ariel's repeatable GitHub page](https://github.com/miracleray/allometry-rodents).
...
$Difference angle = atan(\frac{\text{median estimated slope} - \text{observed slope}}{1 + \text{median estimated slope} * \text{observed slope}}) * frac{180}/\pi$

We can then calculate that for all the slopes.

```{r}
## Function for testing significant slope changes
test.delta.slope <- function(obs.slope, delta.slope, significance) {
    ## Calculating the angle in degrees
    angle <- abs(atan(delta.slope/(1 + obs.slope * (obs.slope + delta.slope))) * 180/pi)
