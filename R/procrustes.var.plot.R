...
#' @param lwd Optional, the line width (by default, \code{lwd = 2})
...
procrustes.var.plot <- function(M1, M2, col = list("grey", "black"), col.val, magnitude = 1, labels = FALSE, axes = FALSE, pt.size = 1, gridPar, col.range, lwd = 2, ...) {
  ...
  if(dim == 2) {
    ...
    graphics::arrows(M1[, 1], M1[, 2], M2[, 1], M2[, 2], length = 0.075, lwd = lwd, col = col_vector)
  ...
  if(dim == 3) {
    ...
    add.segment <- function(i, M1, M2, lwd = lwd, col) {
      rgl::segments3d(rbind(M1[i, ], M2[i, ]), lwd = lwd, col = col[i])
    }

    silent <- sapply(1:nrow(M1), add.segment, M1 = M1, M2 = M2, col = col_vector, lwd = lwd)
